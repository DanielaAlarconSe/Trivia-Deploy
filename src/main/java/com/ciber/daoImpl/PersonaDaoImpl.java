...
public class PersonaDaoImpl implements IPersonaDao {
	...
	public int registrar(Persona persona) {
		String sql = " INSERT INTO public.persona " + " (per_apellido, per_nombre, per_pais_residencia, per_email) "
				+ " VALUES(?, ?, ?, ?); ";
	...
	public List<PersonaDto> obtenerPersonasUsuario() {
		...
		String sql = "select p.per_codigo, p.per_nombre, p.per_apellido, p.per_email, p.per_pais_residencia, p.per_fecha_registro, "
				+ "u.uwd2 , ut.ust_codigo , ut.ust_nombre, count(case when u.usu_estado = 1 then 1 else null end) as usuario, e.ent_codigo, e.ent_nombre from public.persona p "
				+ "left join public.usuario u on p.per_codigo = u.per_codigo "
	...
	public List<PersonaDto> obtenerAspirantesEntidad(Integer entidad) {
		...
		String sql = "select *, u.usu_codigo as usuario from public.usuario u "
				+ "inner join public.usuario_tipo ut on u.ust_codigo  = ut.ust_codigo "
	...
	public List<Persona> obtenerInstructores() {
		String sql = " SELECT p.per_codigo , p.per_apellido, p.per_nombre FROM public.persona p "
				+ " INNER JOIN public.usuario u ON p.per_codigo = u.per_codigo "
